...
class PacketHandler implements Runnable {
    ...
    void broadcastGetReady(ServerThread server, SocketChannel socket) {
        ...
        if(gamenr == 0) {
            ...
            System.out.println("[GAME START] gamenr=" + gamenr + " area=" + area + " room=" + room);
            List cls = clients.getList();
            for(int i=0; i<cls.size(); i++) {
                cl = (Client) cls.get(i);
                if(cl.getArea()==area && cl.getRoom()==room && cl.getSlot()==slot) {
...
//        retval.put((byte)0x04); // not working in area 
    ...
    void sendGSinfo(ServerThread server, SocketChannel socket, Packet ps) {
            ...
            try {
                ...
                if (slotHost != null) {
                    ...
                    if (selfhostIp != null && !selfhostIp.isEmpty()) {
                        ...
                        Logging.println("GSINFO: no selfhost for " + slotHost.getUserID() + ", using central server");
                    }
                }
            } catch (Exception ex) {
                Logging.println("GSINFO: selfhost lookup failed, falling back to central. " + ex.getMessage());
                ipBytes = this.gs_ip;
            }
        }

        byte[] gsinfo = {0x00,0x04, (byte)192, (byte)168, (byte)6, (byte)66,     //IP placeholder
        ...
        Logging.println("GSINFO: sending GameServer IP "
                + (ipBytes[0] & 0xFF) + "." + (ipBytes[1] & 0xFF) + "."
                + (ipBytes[2] & 0xFF) + "." + (ipBytes[3] & 0xFF) + ":8690");
    ...
    void sendEnterAGL(ServerThread server, SocketChannel socket, Packet ps) {
        Packet p;
        Client cl = clients.findClient(socket);
        System.out.println("[ENTER AGL - LEVEL COMPLETED] userid=" + cl.getUserID() + " gamenr=" + cl.gamenumber);
        int gamenr = db.getGameNumber(cl.getUserID());
        cl.gamenumber = gamenr;
    ...
    void sendLeaveAGL(ServerThread server, SocketChannel socket, Packet ps) {
        ...
        cl.setArea(0);
        cl.gamenumber = 0;
        System.out.println("[AGL LEAVE] userid=" + cl.getUserID());
        db.updateClientOrigin(cl.getUserID(), STATUS_LOBBY, 0, 0, 0, true);
    ...
    // this is the protocol function that glues together all the packet functions
